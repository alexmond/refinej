 ...
 * <li>{@link #apply} is the only method that modifies the filesystem, and only when
 * {@code dryRun} is {@code false}.</li>
 * </ul>
 */
public interface RefactoringEngine {
	...
	/**
	 * Compute all source changes needed to move {@code symbol} to {@code newPackageName}.
	 * Does NOT write any files.
	 */
	ChangeSet computeMove(Symbol symbol, String newPackageName);

	/**
	 * Write the changes in {@code changeSet} to disk. Backs up all affected files first.
	 * A no-op when {@code dryRun} is {@code true}.
	 */
