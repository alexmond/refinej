 ...
 *
 * <p>
 * Phase 1: functional stub that invokes Maven's {@code dependency:build-classpath} goal.
 * Gradle support is stubbed for Phase 2. Failure is non-fatal — engines fall back to
...
public class ClasspathResolverImpl implements ClasspathResolver {
	...
	private List<Path> resolveMaven(Path projectRoot) {
		...
		try {
			...
			if (exitCode != 0) {
				log.warn("Maven classpath resolution exited with code {}; using no-classpath mode", exitCode);
				return List.of();
			}
			...
			log.warn("Maven classpath resolution failed: {}; using no-classpath mode", ex.getMessage());
			return List.of();
		}
	}
