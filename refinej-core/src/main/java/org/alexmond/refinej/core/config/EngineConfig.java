 ...
 *
 * <p>All three engine implementations are always on the classpath and instantiated
 * by Spring. This factory picks the one matching {@code refinej.default-engine}
 * (or the per-command {@code --engine} flag via {@code EngineResolver} in refinej-cli).
 *
 * <p>Using a factory over {@code @Profile} keeps all engines in the same JAR.
 */
@Slf4j
@Configuration
public class EngineConfig {
    ...
    public RefactoringEngine activeEngine(List<RefactoringEngine> engines) {
        EngineType target = EngineType.fromString(defaultEngine);

        RefactoringEngine selected = engines.stream()
                .filter(e -> e.getType() == target)
                .findFirst()
                .orElseThrow(() -> new IllegalStateException(
                        "No RefactoringEngine bean found for type: " + target
