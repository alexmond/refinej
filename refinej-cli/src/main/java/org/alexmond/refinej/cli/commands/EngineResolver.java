 ...
 *
 * <p>When {@code --engine} is provided it overrides the configured default for that
 * command only. Otherwise the {@code @Primary} engine from {@code EngineConfig} is used.
 */
@Component
    ...
    public RefactoringEngine resolve(String engineOverride) {
        ...
        return allEngines.stream()
                ...
                .orElseThrow(() -> new IllegalArgumentException(
                        "Unknown engine '" + engineOverride
                        + "'. Valid values: spoon, rewrite, javaparser"));
    }
}
